...
2.0
    ...
    - Read room midpoint and crate object, initialt i x,y,z
    - Read room data into new object

    - Faster read (copy), more robust, att bara filer med valt namn läses in
    - Kunna skala om punkter och ge dem ny geometri
    - Valfri sökterm
    - Remove outlayers som option (0 kopplingar tas alltid bort)
...
#        )
    my_filterlist: EnumProperty(
        ...
        items=[ ('IFCSpace', "IFCSpace", "IFCSpace", 'MOD_WIREFRAME', 0),
                ...
                ('CustomProperty', "Custom Property", "Containing custom property", 'PRESET', 27),
    ...
    my_exportobjectlist: EnumProperty(
        ...
        items=[ #('BuildingBodies', "Create Building bodies from volumes", "Volumes must be closed and made of planar surfaces. Each selected object will create one building body", 'HOME', 0),
                ...
                #('Bitmaps', "Export selected objects as bitmaps", "Export selected object data as bitmaps", 'COLORSET_02_VEC', 1),
               ]
        )
    my_postprocesslist: EnumProperty(
        ...
        items=[ #('CreatePointCSV', "Create Point CSV ", "Fom idm or H5. DF, Point Illuminance. Tricky, needs to be flexible and support different formats", 'MESH_CUBE', 0),
                ...
                ('CreteRoomShape', "Import zone result",  "Create mesh with data for each zone", 'MESH_CUBE', 0),
                ...
                ('ColorAccordingToProperty', "Color Selected Objects According To Property", "Color scale, max, min, type", 'COLORSET_02_VEC', 2),
                ('TextAccordingToProperty', "Text Selected Objects According To Property", "Text According To Property", 'FILE_TEXT', 3),
...
#            #    bpy.context.scene.collection.objects.link(obj)
        if mytool.my_postprocesslist == "CreatePointsFromVTK":
            ...
            bpy.context.active_object.name = 'VTK template'
            ...
            bpy.context.active_object["Connection"] = 0 #Ge alla 0 kopplingar initialt
...
#                new_obj["FirstRow"] = "ZOnejldf"
#                new_obj.location[0] = src_obj.location[0] + i * 2
            #elif str(mytool.my_shapelist) == "Cube":
                #bpy.ops.mesh.primitive_cube_add(location = (float(point [0]), float(point [1]), float(point [2])), size = float(mytool.my_size))
            #elif str(mytool.my_shapelist) == "Sphere":
                #bpy.ops.mesh.primitive_uv_sphere_add(radius = (float(mytool.my_size) / 2), location = (float(point [0]), float(point [1]), float(point [2])))
            #elif str(mytool.my_shapelist) == "Cylinder":
                #bpy.ops.mesh.primitive_cylinder_add(radius = (float(mytool.my_size) / 2), depth=0.01, location = (float(point [0]), float(point [1]), float(point [2])))
            #For each VTK-file in folder, should be only with right name
            for vtkfiles in os.listdir(os.path.dirname(mytool.model_path)):
...
#                                    if str(mytool.my_shapelist) == "Plane":
                                    ...
                                    new_obj["FirstRow"] = "Zone Group"
                                    #new_obj["Connection"] = "0"
                                    new_obj.location[0] = float(point [0])
                                    new_obj.location[1] = float(point [1])
                                    new_obj.location[2] = float(point [2])
                                    templistofpoints.append(new_obj)
                                    #bpy.ops.mesh.primitive_plane_add(location = (float(point [0]), float(point [1]), float(point [2])), size = float(mytool.my_size))
#                                    elif str(mytool.my_shapelist) == "Cube":
#                                        bpy.ops.mesh.primitive_cube_add(location = (float(point [0]), float(point [1]), float(point [2])), size = float(mytool.my_size))
#                                    elif str(mytool.my_shapelist) == "Sphere":
#                                        bpy.ops.mesh.primitive_uv_sphere_add(radius = (float(mytool.my_size) / 2), location = (float(point [0]), float(point [1]), float(point [2])))
#                                    elif str(mytool.my_shapelist) == "Cylinder":
#                                        bpy.ops.mesh.primitive_cylinder_add(radius = (float(mytool.my_size) / 2), depth=0.01, location = (float(point [0]), float(point [1]), float(point [2])))
#                                    #Give points good name
                                    #ob = bpy.context.object
                                    #me = ob.data
#                                    bpy.context.object.name = 'ICA ICE result'
#                                    ob["FirstRow"] = "Zone Name" #Ge alla 0 kopplingar initialt
#                                    ob["SecondRow"] = "Zone Group" #Ge alla 0 kopplingar initialt
#                                    ob["Connection"] = 0 #Ge alla 0 kopplingar initialt
#                                    templistofpoints.append(ob) #Save all points in a temp list
                            elif readstate == "CONNECTIVITY":#ta på vilka punkter som skall ignoreras genom att lagra kopplingar till andra punkter
                                for i in line.split():
                                    templistofpoints[int(i)]["Connection"] = int(templistofpoints[int(i)]["Connection"]) + 1
                            elif readstate == "LOOKUP_TABLE":
                                for i in line.split():
                                    templistofpoints[temppoint]["Result"] = i
                                    temppoint = temppoint + 1
#                   #Delete points depending on number of connections
                    # Deselect all
                    bpy.ops.object.select_all(action='DESELECT')
                    for obj in bpy.context.visible_objects:
                        if obj["Connection"] < 1: #3 or obj["Connection"] > 7: #Delete point fewer than 3 or more than 6. A bit wild I know  
                            obj.select_set(True)
                    bpy.ops.object.delete()
            #Delete the template object
            bpy.ops.object.select_all(action='DESELECT')
            for obj in bpy.context.visible_objects:
                if obj["FirstRow"] == "VTK template":
                    obj.select_set(True)
            bpy.ops.object.delete()
...
#                bpy.context.scene.collection.objects.link(obj)
        if mytool.my_postprocesslist == "AggregatePoints":
            #We do not know what the points represent
            ...
            if str(mytool.my_shapelist) == "Plane":
                bpy.ops.mesh.primitive_plane_add(location = (tempx,tempx,tempz), size = float(mytool.my_size))
            elif str(mytool.my_shapelist) == "Cube":
                bpy.ops.mesh.primitive_cube_add(location = (tempx,tempx,tempz), size = float(mytool.my_size))
            elif str(mytool.my_shapelist) == "Sphere":
                bpy.ops.mesh.primitive_uv_sphere_add(radius = (float(mytool.my_size) / 2), location = (tempx,tempx,tempz))
            elif str(mytool.my_shapelist) == "Cylinder":
